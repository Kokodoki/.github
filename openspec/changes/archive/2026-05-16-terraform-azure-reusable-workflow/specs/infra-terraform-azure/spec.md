...
- **WHEN** la configuración de `backend-config` apunta a un Azure Storage Account o contenedor inexistente
...
### Requirement: Autenticación en Azure mediante OIDC o client secret

El workflow SHALL soportar dos métodos de autenticación en Azure usando `azure/login@v2`: OIDC (Workload Identity Federation, sin client secret) o credenciales estáticas mediante el secret `azure-client-secret`. En ambos casos son requeridos los secrets `azure-client-id`, `azure-tenant-id` y `azure-subscription-id`.

#### Scenario: OIDC autentica correctamente sin client secret

- **WHEN** el repositorio consumidor no provee el secret `azure-client-secret` y la App Registration en Entra ID tiene configurada la credencial federada para el repositorio
- **THEN** la autenticación OIDC es exitosa y los comandos de Terraform pueden acceder a los recursos de Azure

#### Scenario: Client secret autentica correctamente cuando está presente
...
- **THEN** la autenticación con client secret es exitosa y los comandos de Terraform pueden acceder a los recursos de Azure

#### Scenario: Autenticación falla si los secrets requeridos no están configurados

- **WHEN** los secrets `azure-client-id`, `azure-tenant-id` o `azure-subscription-id` no están presentes
...
- **WHEN** un repositorio consumidor invoca el workflow usando `with: terraform-version: "1.9.0"` y `with: working-directory: "infra/azure"`
