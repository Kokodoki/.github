...
- **WHEN** la configuración de `backend-config` apunta a un Storage Account o container inexistente en Azure
...
### Requirement: Job apply condicional mediante flag enable-apply

El workflow SHALL omitir el job `apply` cuando el input booleano `enable-apply` sea `false`.

#### Scenario: Apply omitido cuando enable-apply es false

- **WHEN** el repositorio consumidor invoca el workflow con `enable-apply: false`
- **THEN** el job `apply` no se ejecuta y el workflow termina tras el job `plan`

#### Scenario: Apply se ejecuta cuando enable-apply es true

- **WHEN** el repositorio consumidor invoca el workflow con `enable-apply: true` (o usa el default)
- **THEN** el job `apply` se ejecuta tras el job `plan` y espera aprobación en el Environment

---

### Requirement: Autenticación en Azure mediante OIDC con azure/login

El workflow SHALL autenticar en Azure usando `azure/login@v2` con los inputs `azure-client-id`, `azure-tenant-id` y `azure-subscription-id`. Si el secret `azure-client-secret` está presente, SHALL pasarlo al action para soportar autenticación con Service Principal clásico.

#### Scenario: OIDC autentica correctamente sin client-secret

- **WHEN** el repositorio consumidor provee `azure-client-id`, `azure-tenant-id` y `azure-subscription-id` y tiene Federated Credentials configuradas en Azure AD
- **THEN** la autenticación OIDC es exitosa y los comandos de Terraform pueden acceder a los recursos de Azure

#### Scenario: Autenticación con client-secret funciona como alternativa

- **WHEN** el secret `azure-client-secret` está presente junto con los tres inputs de identidad
- **THEN** el workflow se autentica mediante Service Principal con contraseña

#### Scenario: Autenticación falla si los inputs de identidad están incompletos

- **WHEN** alguno de los inputs `azure-client-id`, `azure-tenant-id` o `azure-subscription-id` está vacío
- **THEN** el step de `azure/login` falla con error de configuración
...
- **WHEN** un repositorio consumidor invoca el workflow usando `with: terraform-version: "1.9.0"` y `with: azure-client-id: "<id>"`
